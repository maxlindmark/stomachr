...
Below I compare the data coverage in the live version of the new database, with the last online version of the old database. The old database is not bundled with this package (it's 131MB). Let me know if you want a copy.

```{r old-path, eval = TRUE}
old_path <- "../my-scripts/StomachDataFullOutput.csv"
has_old_data <- file.exists(old_path)
```

```{r read-old, eval = has_old_data}
old <- readr::read_csv(
  old_path,
...
The old export uses full country names rather than the ISO codes the new database uses, so we map them by hand. It also has no ecoregion column, so we classify it into the same three regions from `Latitude`/`Longitude`: east of 13°E is Baltic Sea; east of 9°E is also Baltic Sea if south of 56°N (this picks up the Danish straits/Kattegat); everything else is Greater North Sea. Note that we therefore merge Celtic Seas and Greater North Sea (because Celtic Seas doesnt have much data and it's not as easy to split based on coordinates...).

```{r old-clean, eval = has_old_data}
country_map <- c(
...
```{r old-year-range, eval = has_old_data}
...
```{r plot-old-new-overall, eval = has_old_data, fig.height = 4, fig.width = 8}
...
```{r plot-old-new-region, eval = has_old_data, results = "asis", fig.height = 3, fig.width = 7}
cmp_region <- cmp |>
...
```{r plot-old-new-country, eval = has_old_data, results = "asis", fig.height = 3, fig.width = 8}
