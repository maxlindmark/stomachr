...
Documented as a unique ID per prey item within one predator, but it isn't always -- usually because two genuinely *different* prey species end up sharing the same `PreySequence` value, not because one item was entered twice.

```{r prey-sequence-diagnostic, eval = TRUE, message = FALSE}
...
- Almost entirely one country's uploads, and almost entirely different species sharing one slot rather than a repeated entry of the same one.
- `join_stomach_data()` flags this (a `cli_warn()` naming the duplicated pair count) but keeps every row -- there's no way to tell from the data alone which entry (if either) is wrong, so nothing is dropped or renumbered automatically.
- **No fix applied in the example vignettes**: unlike the other entries here, this doesn't corrupt anything downstream by itself (every prey row still gets attributed to the right predator) -- it just means `PreySequence` can't be trusted as a real unique key.
...
*"SubFactor"*. Is it subsampling of stomach contents of predators for stomach analysis? further description given. Populated on most prey rows, but `stomachr` currently just carries it through unchanged; nothing multiplies `count`/`weight` by it.

```{r subfactor-diagnostic, eval = TRUE, message = FALSE}
...
- Every populated value seen so far is exactly `1`, so for now it doesn't do anything.
- **No fix applied**: unclear what to do with values that are not 1 if they appear.
